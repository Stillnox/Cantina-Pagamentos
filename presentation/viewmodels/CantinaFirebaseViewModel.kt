package com.cantina.pagamentos.presentation.viewmodels
...
import com.cantina.pagamentos.data.services.ServicoAutenticacao
import com.cantina.pagamentos.data.services.ServicoDados
...
class CantinaFirebaseViewModel : ViewModel() {
                        ...
                        servicoDados.observarClientes()
                            ...
                            .collect { listaClientes ->
                                ...
                                if (!recebeuPrimeiroSnapshot) {
                                    _isCarregando.value =
                                        false // Para o loading após o primeiro snapshot
    ...
    fun criarCliente(
        ...
        viewModelScope.launch {
            ...
            when (val resultado =
                servicoDados.criarCliente(nomeCompleto, dataNascimento, telefone)) {
                is ResultadoFirebase.Sucesso -> {
...
// ===== FUNÇÕES DE GERAÇÃO DE PDF =====
    ...
    fun gerarPdfExtratoCliente(context: Context, clienteId: String): Uri? {
        try {
            ...
            transacoes.sortedByDescending { it.data }.forEach { transacao ->
                ...
                val valor =
                    "${if (transacao.tipo == "CREDITO") "+" else "-"} R$ ${"%.2f".format(transacao.valor)}"
                normalPaint.color =
                    if (transacao.tipo == "CREDITO") 0xFF008000.toInt() else 0xFFFF0000.toInt()
            ...
            // Salva o arquivo
            val fileName = "extrato_${
                cliente.nomeCompleto.replace(
                    " ",
                    "_"
                )
        ...
        try {
            ...
            clientes.forEach { cliente ->
                ...
                if (cliente.saldo < 0) {
                    ...
                    canvas.drawText(
                        "(Limite: R$ ${"%.2f".format(cliente.limiteNegativo)})",
                        250f,
                        yPosition,
                        paint
                    )
    ...
    fun gerarRelatorioCompleto(context: Context): Uri? {
        ...
        try {
            ...
            yPosition += 20
            canvas.drawText(
                "Gerado por: ${getNomeFuncionario()} (ADMIN)",
                50f,
                yPosition,
                normalPaint
            )
            ...
            canvas.drawText(
                "SALDO TOTAL DO SISTEMA: R$ ${"%.2f".format(saldoTotal)}",
                50f,
                yPosition,
                paint
            )
            ...
            clientes.forEach { cliente ->
                ...
                canvas.drawText(
                    "Tel: ${cliente.telefone} | Nasc: ${cliente.dataNascimento}",
                    70f,
                    yPosition,
                    normalPaint
                )
                ...
                paint.color = if (cliente.saldo >= 0) 0xFF008000.toInt() else 0xFFFF0000.toInt()
                canvas.drawText(
                    "Saldo: R$ ${"%.2f".format(cliente.saldo)} | Limite: R$ ${"%.2f".format(cliente.limiteNegativo)}",
                    70f, yPosition, paint
                )
                yPosition += 30
            }
     ...
     * @param uri URI do arquivo PDF a ser compartilhado
     * @param titulo Título que aparecerá no compartilhamento
     */
    fun compartilharPdf(context: Context, uri: Uri, titulo: String) {
        // Cria um Intent para compartilhar arquivo
        val intent = Intent(Intent.ACTION_SEND).apply {
            // ✅ FORMA 1: Usando propriedade (recomendado em Kotlin)
            type = "application/pdf"

            // OU

            // ✅ FORMA 2: Usando método (estilo Java)
            // setType("application/pdf")

            // Adiciona o arquivo PDF como anexo
            putExtra(Intent.EXTRA_STREAM, uri)

            // Define o título/assunto do compartilhamento
            putExtra(Intent.EXTRA_SUBJECT, titulo)

            // Permite que outros apps leiam o arquivo temporariamente
            addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
        }

        // Abre o seletor de apps para compartilhar
